# Research state

The implementation request authorizes M1–M4, superseding the earlier M1-only
boundary without expanding beyond the specified proof-architecture chain.
Lean 4.19.0 core/Std only; no external packages or toolchain changes.

M1 advanced: independently numbered H4 edited witness; rational consistency
model; nonvacuous enclosure interface; constructed s/t²; triangle normalization;
conditional bridge; checked coefficient rearrangements. See M1.md for exact
remaining contact, mechanical and chronology gaps. M1 is not declared complete.

M2 next: finite central-impulse polygon construction and conditional area limit.
M3 follows: finite-curvature contact premises, cubic enclosure and uniform sums.
M4 follows: proposed-revision evidence distinct from actual edition comparison.

Unrelated conversation-export deletion/new file remain untouched.
