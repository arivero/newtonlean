...
| 3 | Time subdivision | Two-cell mismatch, arbitrary finite constant-force endpoint residual/mesh coefficient bound, and within-cell partial-position residual/mesh bound (PartialCell.lean) and an explicit small-residual refinement for each tolerance (UniformRefinement.lean) derived. Next rational-time convergence across partitions; track connectors and absolute polygon-strip defects separately |
